...
class QuestionnairePage extends StatefulWidget {
	...
	final Function(int newCount)? onSessionQuizCountChanged;
		...
		const QuestionnairePage({super.key, required this.loggedUser, required this.sessionQuizCount, this.onSessionQuizCountChanged, this.answers});
...
class _QuestionnairePageState extends State<QuestionnairePage> {
				...
				bool _isCurrentAnswered() {
					...
					if (page is AdvancedQuestion) {
						...
						return answers[keyPos] != null && answers[keyNeg] != null && answers[keyTxt] != null && answers[keyTxt].toString().isNotEmpty;
					} else if (page is Map) {
		...
		for (var i in indices) {
			if (i >= 0 && i < q2.options.length) {
				if (i == q2.options.length - 1) { // Outro
							...
							child: PageView.builder(
								...
								itemBuilder: (context, index) {
									...
									if (page is AdvancedQuestion) {
										...
										if (page.id == 'q03') {
											final q2 = advancedQuestions.firstWhere((q) => q.id == 'q02', orElse: () => AdvancedQuestion(id: '', question: '', type: QuestionType.singleChoice));
											List<int> indices = (answers['q02'] ?? <int>[]) is List ? (answers['q02'] as List).cast<int>() : <int>[];
											List<String> bancosQ2 = [for (var i in indices) if (i >= 0 && i < q2.options.length) q2.options[i]];
										...
										questionWidget = Column(
											...
											children: [
												...
												Slider(
													...
													onChanged: (val) {
														setState(() { answers[keyPos] = val.round(); });
													},
												),
												...
												Slider(
													...
													onChanged: (val) {
														setState(() { answers[keyNeg] = val.round(); });
													},
												),
												TextField(
													decoration: const InputDecoration(labelText: 'Explique o sentimento'),
													onChanged: (val) {
														setState(() { answers[keyTxt] = val; });
													},
												),
											],
										);
						...
						Row(
							...
							children: [
												if (_currentPage > 0)
													ElevatedButton.icon(
														onPressed: () {
															_pageController.previousPage(duration: const Duration(milliseconds: 180), curve: Curves.fastOutSlowIn);
														},
												...
												if (_currentPage < totalPages - 1)
													ElevatedButton.icon(
														onPressed: _isCurrentAnswered()
															? () {
																_pageController.nextPage(duration: const Duration(milliseconds: 180), curve: Curves.fastOutSlowIn);
															}
															: null,
